$(document).ready(function(){
  ...
  $("#txtCommand").bind("enterKey",function(e){
    sendCommand($("#txtCommand").val(), $("#userToken").val());
  });
...
function sendCommand(command, token){
  if (command == "") {
    alertDanger("Вы не ввели команду для выполнения.");
    return;
  }
  logMsg(command, ">", "success");
  $.get(`api/rcon?cmd=${command}&token=${token}`)
    .done(function(json){
      if(json.status){
        if(json.status == 'success' && json.response && json.command){
          if(json.response.indexOf("Unknown command") != -1){
            alertDanger("Неизвестная команда: " + json.command); 
            ...
            alertSuccess("Команда отправлена серверу на обработку.");
            logInfo(json.response);
          }
        }
        else if(json.status == 'error' && json.error){
          if(json.error.indexOf("Command not accepted") != -1){
            alertDanger("Команда отклонена сервером."); 
            logDanger("Команда отклонена сервером.");
          }
          else{
          ...
          alertDanger("Неправильный ответ API RCON"); 
          logDanger("Неправильный ответ API RCON. Пожалуйста, обратитесь в нашу группу ВК.");
        }
      }
      else{
        alertDanger("Ошибка API RCON (статус не возвращен)"); 
        logDanger("Ошибка API RCON (статус не возвращен). Пожалуйста, обратитесь в нашу группу ВК.");
      }
    })
    .fail(function() {
      alertDanger("Ошибка RCON");
      logDanger("Ошибка RCON. Пожалуйста, обратитесь в нашу группу ВК.");
    });
}
